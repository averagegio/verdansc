...
import type { MoveOutChecklistItem } from "../lib/moveOutTracker";

type MoveOutPlanPayload = {
  caseLabel?: string;
...
export default function MoveOutTrackerPage() {
  ...
  const [communicationLogReady, setCommunicationLogReady] = useState(false);
  const [notes, setNotes] = useState("");
  const [supportingImages, setSupportingImages] = useState<File[]>([]);
  const [uploadedImageUrls, setUploadedImageUrls] = useState<string[]>([]);
  const [checklist, setChecklist] = useState<MoveOutChecklistItem[]>([]);
  ...
  const riskWarning = useMemo(() => {
    ...
    const target = new Date(leaseEndDate);
    ...
    if (days <= 7 && days >= 0 && (!ledgerReady || !communicationLogReady)) {
      return "Lease end date is within 7 days and key documentation remains incomplete.";
    }
    return "";
  }, [communicationLogReady, leaseEndDate, ledgerReady]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError("");
      try {
        const response = await fetch("/api/move-out-tracker", {
          method: "GET",
          cache: "no-store",
        });
        const payload = (await response.json()) as {
          ok: boolean;
          message?: string;
          plan?: MoveOutPlanPayload | null;
        };

        if (!response.ok || !payload.ok) {
          setError(payload.message ?? "Could not load move-out tracker.");
          return;
        }

        if (payload.plan) {
          setCaseLabel(payload.plan.caseLabel ?? "");
          setPropertyAddress(payload.plan.propertyAddress ?? "");
          setNoticeDate(payload.plan.noticeDate ?? "");
          setLeaseEndDate(payload.plan.leaseEndDate ?? "");
          setLedgerReady(payload.plan.ledgerReady);
          setNoticeServed(payload.plan.noticeServed);
          setCommunicationLogReady(payload.plan.communicationLogReady);
          setNotes(payload.plan.notes ?? "");
          setUploadedImageUrls(payload.plan.evidenceImages ?? []);
          setChecklist(payload.plan.checklist ?? []);
        }
      } catch {
        setError("Could not load move-out tracker.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (supportingImages.length > 0) {
        ...
        uploadForm.set("context", "move-out-tracker");
      ...
      const payload = (await response.json()) as {
        ok: boolean;
        message?: string;
        plan?: MoveOutPlanPayload;
      };

      if (!response.ok || !payload.ok || !payload.plan) {
        setError(payload.message ?? "Could not save move-out tracker.");
        return;
      }
      ...
      setSupportingImages([]);
      setMessage("Move-out tracker saved.");
    } catch {
      setError("Could not save move-out tracker.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <section className="mx-auto grid w-full max-w-6xl gap-6 md:grid-cols-[1.4fr_1fr]">
        <article className="rounded-2xl border border-cyan-400/30 bg-slate-900/65 p-7 shadow-2xl shadow-cyan-950/30">
          <p className="text-xs uppercase tracking-[0.2em] text-cyan-300">
            Landlord Move-Out Tracker
          </p>
          <h1 className="mt-2 text-3xl font-semibold md:text-4xl">
            Keep move-out case records organized
          </h1>
          <p className="mt-4 max-w-3xl text-slate-300">
            Track key documentation and timeline readiness for move-out and potential
            eviction-prep workflows.
          </p>
          ...
          <p className="mt-3 rounded-lg border border-amber-300/35 bg-amber-900/15 px-3 py-2 text-xs text-amber-100">
            This tracker helps organize records and deadlines. It is not legal advice.
          </p>
          ...
          <form className="mt-6 space-y-3" onSubmit={onSubmit}>
            <input
              ...
              type="text"
              placeholder="Case label (example: Unit 4B - June 2026)"
              value={caseLabel}
            ...
            <label className="flex items-center gap-2 text-sm text-slate-200">
              <input
                type="checkbox"
                checked={ledgerReady}
                onChange={(event) => setLedgerReady(event.target.checked)}
              />
              Rent ledger summary prepared
            </label>
            <label className="flex items-center gap-2 text-sm text-slate-200">
              <input
                type="checkbox"
                checked={noticeServed}
                onChange={(event) => setNoticeServed(event.target.checked)}
              />
              Notice served and documented
            </label>
            <label className="flex items-center gap-2 text-sm text-slate-200">
              <input
                type="checkbox"
                checked={communicationLogReady}
                onChange={(event) => setCommunicationLogReady(event.target.checked)}
              />
              Communication log compiled
            </label>
            <textarea
              ...
              placeholder="Notes (timeline checkpoints, filing prep notes, property coordination)"
              value={notes}
            ...
            <div className="space-y-2 rounded-md border border-cyan-300/30 bg-slate-950/45 px-3 py-3">
              ...
              <p className="text-xs text-slate-300">
                Add photos for notice records, property condition, or supporting case files.
              </p>
              <input
                type="file"
                accept="image/*"
        ...
        <aside className="rounded-2xl border border-cyan-400/30 bg-slate-900/65 p-7">
          <h2 className="text-lg font-semibold text-cyan-100">Case readiness checklist</h2>
          ...
          <div className="mt-4 space-y-2">
            {(checklist.length
              ...
              : [
                  {
                    id: "placeholder",
                    label: "Save your tracker to generate checklist tasks.",
                    complete: false,
