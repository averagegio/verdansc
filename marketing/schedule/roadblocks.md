...
| 4 | **CAN-SPAM still missing.** Need a real `[PHYSICAL_MAILING_ADDRESS]` and a working `[UNSUB_URL]` before any commercial email. | Skip every `email` slot until both exist. | Paste address + unsub URL, or confirm skip? |
...
| 6 | **Stay on the calendar.** 5 slots/day, 7:00 / 7:40 / 8:20 / 9:00 / 9:40 America/Denver, Sep 3–9. Do not blast all groups or inboxes in one sitting. | One unique SB group per day. One Craigslist housing-wanted per day. Inbound Facebook DMs only. | OK? |
...
- No email sent (including from Zoho).
- No Facebook post, comment, or DM.
- No Craigslist post.
- Craigslist login-link email to `mrigwe234@gmail.com` was **not** opened or clicked.
- No extra email scrape.

Reply on this file or in chat: yes/no per row 1–6, plus CAN-SPAM values if email should ever leave `queued`.
